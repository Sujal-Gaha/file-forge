...
def convert_document(
    input_path: Path, output_format: str, output_path: Optional[Path] = None
) -> Path:
    """
...
def extract_pdf_pages(
    input_path: Path, output_path: Path, start_page: int, end_page: Optional[int] = None
) -> Path:
    """
    ...
    with open(input_path, "rb") as pdf_file:
        ...
        if start_idx < 0 or start_idx >= total_pages:
            raise ValueError(
                f"Start page {start_page} is out of range (1-{total_pages})"
            )
